...
function getSteamInventory($id, $items){
    global $Functions;
	$json = json_decode(file_get_contents("http://steamcommunity.com/id/kasieris/inventory/json/730/2?l=russian"),true);
...
switch($action){
	...
	case 'updateLink':
		...
		if(!preg_match('/^(https?:\/\/)?(http?:\/\/)?(www\.)?steamcommunity.com\/tradeoffer\/new\/\?partner=([0-9]+)&token=([A-Za-z0-9\-\_]+)/', $link) || empty($link)){
              echo json_encode(['result' => 'false', 'message' => 'Enter valid trade url!', 'url' => $link]);
		}else{
            $Functions->db->query("UPDATE users SET tradelink='".$link."' WHERE steamid = '".$user->steamid."'");
          echo json_encode(['result' => 'true', 'message' => 'Trade url saved!']);
		}
	break;
	case 'sellItem':
  ...
  case 'resendItem':
      $itemid = intval($_REQUEST['item_id']);
      if($itemid > 0){
          $item = $Functions->db->query("SELECT * FROM drops WHERE id = '".$itemid."' AND status = '4' AND user = '".$user->steamid."'");

          if($item->num_rows == 0){
              echo json_encode(['result' => 'false', 'message' => 'Ошибка! Предмет не найден.']);
              header("Location: /epicdrop/itemserror.php");
          }else{
            header("Location: /epicdrop/itemserror.php");
              $item = $item->fetch_object();
  ...
  case 'sendItem2':
      $itemid = intval($_REQUEST['item_id']);
      if($itemid > 0){
          $item = $Functions->db->query("SELECT * FROM drops WHERE id = '".$itemid."' AND status = '5' AND user = '".$user->steamid."'");
  ...
  break;
    ...
    break;
    case 'checkCase':
        $caseName1 = $Functions->getString($_POST['case1']);
	       $case1 = $Functions->db->query("SELECT * FROM cases WHERE name = '".$caseName1."'")->fetch_object();
         $lapatest = $case1->items;
            $items1 = explode(",", $case1->items);
            ...
            if($inv1 == false){
                $Functions->db->query("UPDATE cases SET visible='0' WHERE name = '".$caseName1."'");
              echo json_encode(['result' => 'false', 'message' => 'no_items11' , 'case' => $caseName1]);
            }else{
              $Functions->db->query("UPDATE cases SET visible='1' WHERE name = '".$caseName1."'");
              echo json_encode(['result' => 'true', 'message' => 'ir_items', 'case' => $caseName1, 'items' => $randItems1, 'lala' => $lapatest]);
        }
    break;
    case 'openCase':
            ...
            echo json_encode(['result' => 'false', 'message' => 'no_login']);
                ...
                $case3 = $Functions->db->query("SELECT * FROM cases WHERE name = '".$caseName."' and visible='1'");
					...
					if($user->money < $case->price){
						...
						if($inv == false){
              //  $Functions->db->query("UPDATE cases SET visible='0' WHERE name = '".$caseName."'");
							echo json_encode(['result' => 'false', 'message' => 'no_items']);
						}else{
            //  $Functions->db->query("UPDATE cases SET visible='1' WHERE name = '".$caseName."'");
              ...
              //  $n = $Functions->db->query("SELECT name FROM items WHERE itemid = '".$randItem['id']."'")->fetch_object();
				        $item2 = $Functions->db->query("SELECT name FROM items WHERE id = '".$randItem['id']."'")->fetch_object();

							$Functions->db->query("INSERT INTO `drops`(`weapon_name`, `case_name`, `itemid`, `inventoryid`, `weaponid` , `casename`, `price`, `user`, `time`, `status`) VALUES ('".$item2->name."','".$case->title."' ,'".$randItem['id']."', '".$randItem['inventory_id']."', '".$randItem['weaponid']."', '".$case->name."', '".$price."', '".$user->steamid."', '".time()."', '0')");
							echo json_encode(
								[
									'result' => 'true',
									'items' => $itemsjson,
