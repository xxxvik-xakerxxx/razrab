...
$_SESSION['urlpage'] = " <a href='index.php'>Главная</a> / <a>Кейсы</a>";
...
switch ($action) {
case 'delete':
$delete = mysql_query("DELETE FROM cases WHERE id='$id'",$bd);
break;
}
}
?>
        ...
        <div id="wrapper">
            ...
            <div class="sidebar-collapse menu-scroll">
                <ul id="side-menu" class="nav">
                    ...
                    <li class="active"><a href="cases.php"><i class="fa fa-suitcase fa-fw">
...
?>
<span class="h2class"><button type="button" class="btn btn-success" onclick="javascript:window.location='add_cases.php'">Добавить кейс</button> <span class="badge bg-theme"><b><?php echo $all_count_rez; ?></b></span>
...
$count = mysql_query("SELECT COUNT(*) FROM cases $cat",$bd);
...
if ($temp[0] > 0)
{
$result = mysql_query("SELECT * FROM cases ORDER BY id DESC LIMIT $start, $num",$bd);
...
$img_path   = '/template/img/'.$row["picture"];
echo '<div class="col-sm-6 col-md-3"><div class="visotaDiva">
                                        <div class="thumbnail">  <img src="'.$img_path.'" width="150" height="150" "/>

                                            <div class="caption"><h3>'.$row["title"].'</h3>

                                                <p>'.$row["price"].' .руб</p>

                                                <p><a  role="button" class="btn btn-primary" href="edit_cases.php?id='.$row["id"].'">Изменить</a>  <a role="button" class="btn btn-danger" href="cases.php?'.$url.'id='.$row['id'].'&action=delete" class="delete">Удалить</a></div>
                                        </div></div></div>
                                    ';
